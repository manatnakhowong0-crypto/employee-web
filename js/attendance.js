...
const AttendancePage = (() => {
  ...
  function _saveToStorage() {
    try { localStorage.setItem(LS_KEY, JSON.stringify(_overrides)); } catch {}
  }
  ...
  function _renderSelectors(employees) {
    ...
    // hint bar (แสดงเฉพาะเมื่อล็อกอินแล้ว และเลือกพนักงานคนเดียว)
    const hint = document.getElementById("edit-hint");
    const canEdit = (typeof Auth !== "undefined") && Auth.isLoggedIn();
    if (hint) hint.style.display = (_selectedEmp !== "all" && canEdit) ? "flex" : "none";
  }
  ...
  function openModal(name, year, month, day, currentTimeRaw) {
    // 🔐 ต้องเข้าสู่ระบบก่อนจึงจะแก้ไขได้
    if (typeof Auth !== "undefined" && !Auth.isLoggedIn()) {
      Auth.openLogin(() => openModal(name, year, month, day, currentTimeRaw));
      return;
    }
    _removeModal();
  ...
  async function saveOverride(name, year, month, day) {
    ...
    // อัปเดต local state ก่อน (UI ตอบสนองทันที)
    _overrides[name]                   ??= {};
    ...
    _removeModal();
    render();
    _toast("💾 กำลังบันทึกลง Google Sheet...");

    // POST ไปยัง Google Sheet → แก้ไข "ชีต1" ถาวร
    try {
      const res = await API.writeAttendance({ name, year, month, day, timeStr });
      const ok  = await _isOk(res);
      if (ok) {
        // ดึงข้อมูลจริงจากชีตกลับมา (ชีตเป็นแหล่งข้อมูลหลัก) แล้วล้าง override ชั่วคราว
        const refreshed = await _refreshFromServer();
        if (refreshed) _clearOverride(name, year, month, day);
        render();
        _toast("✅ บันทึกลงชีตเรียบร้อยแล้ว");
      } else {
        _toast("⚠️ บันทึกลงชีตไม่สำเร็จ — เก็บไว้ในเครื่องชั่วคราว", true);
      }
    } catch (err) {
      console.error("[Attendance] saveOverride POST error:", err);
      _toast("⚠️ เชื่อมต่อเซิร์ฟเวอร์ไม่ได้ — เก็บไว้ในเครื่องชั่วคราว", true);
    }
  }

  async function deleteOverride(name, year, month, day) {
    // อัปเดต local state ก่อน
    _overrides[name]                   ??= {};
    _overrides[name][year]             ??= {};
    _overrides[name][year][month]      ??= {};
    ...
    _removeModal();
    render();
    _toast("🗑 กำลังลบข้อมูลออกจาก Google Sheet...");

    // POST ลบไปยัง Google Sheet (timeStr = null = ลบแถวของวันนั้นออกจากชีต1)
    try {
      const res = await API.writeAttendance({ name, year, month, day, timeStr: null });
      const ok  = await _isOk(res);
      if (ok) {
        const refreshed = await _refreshFromServer();
        if (refreshed) _clearOverride(name, year, month, day);
        render();
        _toast("✅ ลบข้อมูลออกจากชีตเรียบร้อยแล้ว");
      } else {
        _toast("⚠️ ลบไม่สำเร็จ — เก็บไว้ในเครื่องชั่วคราว", true);
      }
    } catch (err) {
      console.error("[Attendance] deleteOverride POST error:", err);
      _toast("⚠️ เชื่อมต่อเซิร์ฟเวอร์ไม่ได้ — เก็บไว้ในเครื่องชั่วคราว", true);
    }
  }

  // ── ตรวจผลลัพธ์ POST ──────────────────────────────────────────
  async function _isOk(res) {
    if (!res) return false;
    try {
      const j = await res.clone().json();
      return j?.status === "ok" || j?.status === "success" || res.ok;
    } catch { return !!res.ok; }
  }

  // ── ดึงข้อมูลจริงจากชีตกลับมาใหม่ ─────────────────────────────
  async function _refreshFromServer() {
    try {
      API.invalidate();
      const fresh = await API.fetchAll(true);
      if (fresh && Array.isArray(fresh.attendance)) { _data = fresh.attendance; return true; }
      return false;
    } catch (e) {
      console.warn("[Attendance] refresh error:", e);
      return false;
    }
  }

  // ── ล้าง override ของเซลล์เดียว (หลังชีตบันทึกจริงแล้ว) ────────
  function _clearOverride(name, year, month, day) {
    try {
      if (_overrides?.[name]?.[year]?.[month]) {
        delete _overrides[name][year][month][day];
      }
      _saveToStorage();
    } catch {}
  }
  ...
  function _toast(msg, isWarn) {
    ...
    if (!t) {
      ...
      t.style.cssText = "position:fixed;left:50%;bottom:24px;transform:translateX(-50%);" +
        "z-index:10000;padding:11px 18px;border-radius:10px;font-size:13px;font-weight:600;" +
        "font-family:inherit;box-shadow:0 8px 30px rgba(0,0,0,.25);max-width:90vw;text-align:center;";
      document.body.appendChild(t);
    }
    t.style.background = isWarn ? "#fef2f2" : "#0A2540";
    t.style.color      = isWarn ? "#dc2626" : "#fff";
    t.style.border     = isWarn ? "1px solid #fecaca" : "none";
    t.textContent = msg;
    t.style.opacity = "1";
    clearTimeout(_toastTimer);
    _toastTimer = setTimeout(() => { t.style.transition = "opacity .4s"; t.style.opacity = "0"; }, 2600);
  }
  ...
  async function init(attendanceData) {
    ...
    _loadFromStorage();   // ← โหลด overrides จาก localStorage ก่อน render
    render();
  }
