const AttendancePage = (() => {
  ...
  function _countMonth(empMonthDays, year, month) {
    ...
    for (let d = 1; d <= dim; d++) {
      if (_isFuture(year, month, d)) continue;   // วันที่ยังมาไม่ถึง — ไม่นับ
  ...
  function _cell(rec, name, year, month, day) {
    if (_isFuture(year, month, day)) return `<td class="att-future" title="ยังมาไม่ถึง"></td>`;
